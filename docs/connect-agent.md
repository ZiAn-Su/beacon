...
重启 Claude Code, `/mcp` 列表里出现 `beacon`, 拥有 19 个工具:

- **核心 5**: `register_session` / `notify_human` / `ask_human` / `update_status` / `check_inbox`
- **群聊 4**: `list_channels` / `post_channel` / `ask_channel` / `answer_channel`
...
| `spawn_agent` | **是** | 启动一个新 agent (独立任务/工作目录) 加入 Beacon; 受监护人 allow/ask/deny 控制 (待批则阻塞) |
