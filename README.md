...
A dot shows whether each agent is **running** (recently talked to Beacon) or **offline**.
If you message an offline agent, Beacon can **relaunch it** in its work path
(`claude --continue --print`, your message delivered on stdin) so it drains its
inbox and continues. Auto-wake is **off by default** — a relaunched agent runs with
real tool permissions, so you opt in via `BEACON_WAKE`:

- `BEACON_WAKE=off` (default) — no auto-launch; messages queue until the agent runs.
- `BEACON_WAKE=full` — fully autonomous (`claude --permission-mode bypassPermissions`):
  the woken agent can read its inbox, reply, and continue real work. One inbound
  message launches an autonomous run (real actions, token cost).
- `BEACON_WAKE=acceptEdits|default|plan|bypassPermissions` — pass a specific Claude
  permission mode. `BEACON_WAKE_CMD` overrides the whole relaunch command.

`--continue` reuses the conversation's prior context; the permission mode is what you
set here. The per-runtime recipe lives in `src/server/wake.ts`.
...
圆点显示每个 agent 是**运行中**(刚和 Beacon 交互过)还是**离线**。给离线的 agent 发消息时,
Beacon 可以在它的工作目录里**把它重新拉起来**(`claude --continue --print`,你的消息经 stdin 送达),
它读收件箱、据此继续。被唤醒的 agent 会拿到真实工具权限,所以**自动唤醒默认关闭**,用 `BEACON_WAKE` 开启:

- `BEACON_WAKE=off`(默认)—— 不自动启动;消息排队等 agent 自己运行时收到。
- `BEACON_WAKE=full` —— 完全自主(`claude --permission-mode bypassPermissions`):被唤醒的 agent
  能读收件箱、回复、继续真实干活。**一条入站消息就会启动一次全自主运行(真实动手、消耗额度)。**
- `BEACON_WAKE=acceptEdits|default|plan|bypassPermissions` —— 指定某个 Claude 权限模式;
  `BEACON_WAKE_CMD` 可整体覆盖唤醒命令。

`--continue` 复用该对话**之前的上下文/任务**;权限模式由这里的设置决定。每种运行时的唤醒方式
写在 `src/server/wake.ts`。
