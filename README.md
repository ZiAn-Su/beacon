...
If you message an agent that **isn't running**, Beacon offers to start it for you —
right in the conversation, one click ("Start it & handle this"), with an "always start
automatically" checkbox to remember. It relaunches the agent in its work path
(`claude --continue --print`, your message delivered on stdin), so it reads its inbox
and continues. No env vars: the behaviour is a plain choice in **Settings** (gear icon)
— *Ask me first* (default) / *Start it automatically* / *Just queue*.

`--continue` reuses the conversation's prior context; the started agent runs with the
configured permission mode (default `bypassPermissions` so it can actually act). The
per-runtime recipe lives in `src/server/wake.ts`; `BEACON_WAKE_CMD` is an advanced
override of the launch command.
...
圆点显示每个 agent 是**运行中**(刚和 Beacon 交互过)还是**离线**。给一个**没在运行**的 agent
发消息时,Beacon 会在对话里直接给你一个**一键「启动并处理」**按钮(可勾「以后自动启动」记住选择)。
点一下就把它在工作目录里启动起来(`claude --continue --print`,你的消息经 stdin 送达),读收件箱、接着干。
**没有环境变量**:行为是**设置面板(齿轮图标)**里的一个大白话选项 —— 「先问我(默认)」/「自动启动它」/
「只把消息排队」。

`--continue` 复用该对话**之前的上下文/任务**;启动的 agent 按设置的权限模式运行(默认 `bypassPermissions`,
让它能真正干活)。每种运行时的启动方式写在 `src/server/wake.ts`;`BEACON_WAKE_CMD` 是高级的命令覆盖项。
