...
If you message an offline agent, Beacon **relaunches it** in its work path
(`claude --continue --print`, your message delivered on stdin), it drains its inbox
and continues — fully automatic, nothing to configure (the per-runtime recipe lives
in `src/server/wake.ts`). `BEACON_WAKE=0` disables it; `BEACON_WAKE_CMD` overrides the
relaunch command.
...
Beacon 会在它的工作目录里**把它重新拉起来**(`claude --continue --print`,你的消息经 stdin 送达),
它读收件箱、据此继续 —— **全自动、零配置**(每种运行时的唤醒方式写在 `src/server/wake.ts`)。
`BEACON_WAKE=0` 可关闭;`BEACON_WAKE_CMD` 可覆盖唤醒命令。
